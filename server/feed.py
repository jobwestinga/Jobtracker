...
CREATE INDEX IF NOT EXISTS idx_change_feed_seq ON change_feed(seq);
...
def install(connection: sqlite3.Connection) -> None:
    ...
    for table in sync_policy.SYNCED_TABLES:
        ...
        # The uid trigger fires an UPDATE right after every INSERT to fill in a
        # missing uid, which lands here with the uid populated. The duplicate
        # event is harmless: the client upserts the same row twice, and an upsert
...
def changes_since(
    ...
    for entry in entries:
        ...
        if entry["op"] == "upsert":
            row = cur.execute(
                f"SELECT * FROM {table} WHERE id = ?", (entry["row_id"],)
            ).fetchone()
        else:
            row = None
        ...
        if entry["op"] == "upsert":
            if row is None:
                ...
                continue
            item["row"] = sync_policy.to_wire(database, table, dict(row))
        changes.append(item)
